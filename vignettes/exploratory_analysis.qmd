...
```{r}
sensors <- unique(obs[which(!(obs$network %in% c("HEATWATCH", "ECONET"))),
...
```{r}
idx <- 2
mypair <- obs[which(obs$site_id %in% c(pairs[idx, ]$cws_a, pairs[idx, ]$cws_b)), ]
```
...
```{r}
obs <- obs[which(obs$temp >= -20), ]
wu <- wu[which(wu$temp >= -20), ]
```
