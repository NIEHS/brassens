...
nc_inv$ts_utc <- as.POSIXct(nc_inv$ts_utc, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
nc_inv$te_utc <- as.POSIXct(nc_inv$te_utc, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")

imp_path <- "../input/US_2019_imperviousness/nlcd_2019_impervious_l48_20210604.img"
imp_us <- terra::rast(imp_path) |>
  terra::aggregate(fact = 20, fun = "mean", na.rm = TRUE) |>
  terra::project("epsg:4326") 
nc_shp_proj <- terra::project(nc_shp, "epsg:4326") 
imp_nc <- terra::crop(imp_us, nc_shp_proj, overwrite = TRUE, mask = TRUE, snap = "out") 
```
...
saveRDS(ghcnh,
        paste0("./output/case_study_nc_jja2021/ghcnh_nc_",
...
```{r}
ghcnh <- readRDS(paste0("../output/case_study_nc_jja2021/ghcnh_nc_",
               lubridate::date(config$ts),
               "_",
               lubridate::date(config$te),
               ".rds"))
...
for (i in 1:(length(batches) - 1)) {
  ...
  # save result
  saveRDS(wu_list_cc, paste0("./output/case_study_nc_jja2021/wu_nc_cleaned_and_calibrated_",
               lubridate::date(ts),
               "_",
               lubridate::date(te),
               ".rds"))
...
```{r}
nc <- nc_shp |> terra::project(y = "epsg:32119") # linear unit of nc_shp in meters
...
ggplot2::ggplot() +
  ...
  tidyterra::geom_spatvector(data = nc, color = "white", linewidth = .5, fill = NA) +
  tidyterra::geom_spatvector(data = v, color = "red", linewidth = .5, fill = NA) + 
...
```{r}
f <- "../output/case_study_nc_jja2021/wu_nc_cleaned_and_calibrated_2021-06-01_2021-06-15.rds"
...
for (p in period) {
 ...
 plot <- ggplot() + 
    ...
    geom_sf(data = wu_cc[which(wu_cc$time == p), ],
 ...
 ggsave(plot = plot,
        filename = paste0("../output/case_study_nc_jja2021/graphs/pmap_", 
...
```{r}
saveRDS(wu_list_cc, file = paste0("../output/wu_nc_cleaned_calibrated_",
                           lubridate::date(config$ts),
                           "_",
                           lubridate::date(config$te),
                           ".rds"))
