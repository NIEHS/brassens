compute_daily_trend <- function(cws) {
  ...
  cws_ts <- cws_ts |>
    group_by(
      time = lubridate::floor_date(time, "1 hour"),
      id = id,
      lat = lat,
      lon = lon
    ) |>
  ...
  cws_trend_dailycycle <- cws_trend |>
    ...
    dplyr::summarize(
      temp_med = median(temp_med, na.rm = T),
  ...
  cws_day <- cws_ts |>
    ...
    dplyr::summarize(
      d_temp_min = median(d_temp_min, na.rm = T),
...
mat.dist.cws <- function(cws) {
  ...
  cws.sp <- aggregate(cbind(lon = lon, lat = lat) ~ id,
                      data = cws.sp,
                      FUN = "mean")
  coordinates(cws.sp) <- ~ lon + lat
  proj4string(cws.sp) <- sp::CRS("EPSG:4326")
...
add_neigh.info.cws <- function(cws, k = NA, radius) {
  ...
  for (i in seq(1, length(time), by = 1)) {
    ...
    cws.h <- cws[which((between(cws$time,
                                hour,
                                hour + hours(1) - seconds(1)))), ]
